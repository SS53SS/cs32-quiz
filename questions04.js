...
quizDataRaw = [
    ...
    {
        "text": "データ送信のたびに毎回同じ問い合わせを繰り返すのは無駄なため、一度問い合わせたIPアドレスとMACアドレスの対応関係を「　　　　　」というリストに保存し、次からはこのリストを参照して対応アドレスを求める",
        "choices": ["Routeテーブル", "ARPテーブル", "Listテーブル", "ICMPテーブル"],
        "correctIndex": 1
    },
    {
        "text": "ARPテーブルとはIPアドレスからMACアドレスを調べた際に作られる対応表で、「　　　　　」におけるIPアドレスと対応するMACアドレスを列挙したもの。",
        "choices": ["異なるサブネット間", "同一ネットワーク内", "VPNトンネル内", "ルータ内部のみ"],
        "correctIndex": 1
    },
    {
        "text": "ARPとは「　　　　　」",
        "choices": ["宛先IPアドレスを基に宛先MACアドレスを調べる", "宛先MACアドレスを基に宛先IPアドレスを調べる", "宛先ポートを基に宛先MACアドレスを調べる", "宛先MACアドレスを基に宛先ポートを調べる"],
        "correctIndex": 0
    },
    {
        "text": "ARPの動作は「　　　　　」,ARP応答,ARPキャッシュへの記録である",
        "choices": ["ARP要求", "ARP応答", "ARPキャッシュ", "MACアドレス要求", "IPアドレス通知", "DNS問い合わせ", "ブロードキャスト転送"],
        "correctIndex": 0
    },
    {
        "text": "ARPの動作はARP要求,「　　　　　」,ARPキャッシュへの記録である",
        "choices": ["ARP要求", "ARP応答", "ARPキャッシュ", "MACアドレス要求", "IPアドレス通知", "DNS問い合わせ", "ブロードキャスト転送"],
        "correctIndex": 1
    },
    {
        "text": "ARPの動作はARP要求,ARP応答,「　　　　　」への記録である",
        "choices": ["ARP要求", "ARP応答", "ARPキャッシュ", "MACアドレス要求", "IPアドレス通知", "DNS問い合わせ", "ブロードキャスト転送"],
        "correctIndex": 2
    },
    {
        "text": "ARP要求とは全ホストに対して「　　　　　」通信でARP要求を送信します",
        "choices": ["マルチキャスト", "ブロードキャスト", "全二重", "半二重", "ポイントツーポイント", "ユニキャスト"],
        "correctIndex": 1
    },
    {
        "text": "ARP要求を受信したホストの内、要求内容が合致したホストは、自分のMACアドレスとIPアドレスを格納し、「　　　　　」により、ARP要求元へ応答します",
        "choices": ["マルチキャスト", "ブロードキャスト", "全二重", "半二重", "ポイントツーポイント", "ユニキャスト"],
        "correctIndex": 5
    },
    {
        "text": "HTTPのポート番号は何番か",
        "choices": ["23", "25", "53", "80", "110", "443"],
        "correctIndex": 3
    },
    {
        "text": "HTTPのレスポンス（Response）に関する説明として正しいものはどれですか。",
        "choices": ["サーバーからブラウザへの「お返事」。データと共に処理結果を示す番号が含まれる", "ブラウザからサーバーへの「お願い」", "Wiresharkでは、自分のIPからサーバIPへのパケットとして観察される", "DNSサーバにIPアドレスを問い合わせる通信", "HTTP通信を暗号化するための仕組み"],
        "correctIndex": 0
    },
    {
        "text": "HTTPのリクエスト（Request）に関する説明として正しいものはどれですか。",
        "choices": ["サーバーからブラウザへの「お返事」。データと共に処理結果を示す番号が含まれる", "ブラウザからサーバーへの「お願い」。Wiresharkでは、自分のIPからサーバIPへのパケットとして観察される", "DNSサーバにIPアドレスを問い合わせる通信", "HTTP通信を暗号化するための仕組み"],
        "correctIndex": 1
    },
    {
        "text": "HTTPに関する説明として最も適切なものはどれですか。",
        "choices": ["Webページを転送するアプリケーションプロトコル", "IPアドレスからMACアドレスを調べるプロトコル", "ファイルを暗号化するための通信規格", "メールを送信するためだけに使用されるプロトコル", "ルータ間で経路情報を交換するプロトコル", "ドメイン名をIPアドレスに変換するプロトコル"],
        "correctIndex": 0
    },
    {
        "text": "次のHTTPレスポンスのステータスコード種別から正しい意味を選択してください。Status Code: 200",
        "choices": ["リクエストされたリソースに対して複数の選択肢があるため選択が必要", "クライアント側の要求に誤りがありサーバ側が応えることができない", "サーバ側に原因があり、クライアント側の要求に応えられない", "リクエストは正常に受理されました"],
        "correctIndex": 3
    },
    {
        "text": "次のHTTPレスポンスのステータスコード種別から正しい意味を選択してください。Status Code: 301",
        "choices": ["リクエストされたリソースに対して複数の選択肢があるため選択が必要", "クライアント側の要求に誤りがありサーバ側が応えることができない", "サーバ側に原因があり、クライアント側の要求に応えられない", "リクエストは正常に受理されました"],
        "correctIndex": 0
    },
    {
        "text": "次のHTTPレスポンスのステータスコード種別から正しい意味を選択してください。Status Code: 404",
        "choices": ["リクエストされたリソースに対して複数の選択肢があるため選択が必要", "クライアント側の要求に誤りがありサーバ側が応えることができない", "サーバ側に原因があり、クライアント側の要求に応えられない", "リクエストは正常に受理されました"],
        "correctIndex": 1
    },
    {
        "text": "次のHTTPレスポンスのステータスコード種別から正しい意味を選択してください。Status Code: 500",
        "choices": ["リクエストされたリソースに対して複数の選択肢があるため選択が必要", "クライアント側の要求に誤りがありサーバ側が応えることができない", "サーバ側に原因があり、クライアント側の要求に応えられない", "リクエストは正常に受理されました"],
        "correctIndex": 2
    },
    {
        "text": "１回目と２回目の通信の要求（Request）と応答（Response）は独立している。HTTPの基本的な特徴として「　　　　　」が挙げられる。",
        "choices": ["コネクションレス", "リアルタイム通信", "マルチキャスト通信", "ステートレス", "セッション同期型"],
        "correctIndex": 3
    },
    {
        "text": "Cookieとは通信の状態の「　　　　　」を支えるしくみ",
        "choices": ["終端", "継続", "暗号化", "分割", "初期化", "高速化"],
        "correctIndex": 1
    },
    {
        "text": "CookieにおいてサーバーはセッションIDを発行する。そのヘッダ名は何か",
        "choices": ["Cookie", "Session-ID", "Authorization", "User-Agent", "Set-Cookie"],
        "correctIndex": 4
    },
    {
        "text": "Cookieにおいて、クライアントのブラウザが保存されたセッションIDをサーバへ送信する際に使用するヘッダ名は何か",
        "choices": ["Cookie", "Session-ID", "Authorization", "User-Agent", "Set-Cookie"],
        "correctIndex": 0
    },
    {
        "text": "プロキシの代表的な２つの形態において、クライアント側に設置するのはどれか",
        "choices": ["リバースプロキシ", "フォワードプロキシ", "ロードバランサ", "キャッシュサーバ"],
        "correctIndex": 1
    },
    {
        "text": "プロキシの代表的な２つの形態において、サーバー側に設置するのはどれか",
        "choices": ["リバースプロキシ", "フォワードプロキシ", "ロードバランサ", "キャッシュサーバ"],
        "correctIndex": 0
    },
    {
        "text": "SSL/TLSのポート番号は何番か",
        "choices": ["23", "25", "53", "80", "110", "443"],
        "correctIndex": 5
    },
    {
        "text": "SSL/TLSにおいて、Wiresharkで暗号化された通信の中身が確認できない場合、Info欄にはどのように表示されることが多いか",
        "choices": ["Encrypted", "Application Data", "Secure Payload", "Cipher Text"],
        "correctIndex": 1
    },
    {
        "text": "DNSのポート番号は何番か",
    ...
    {
        "text": "DNS Responseはサーバからの回答パケット。「IPアドレスは〇〇です」という情報の他、エラー情報（名前が存在しない等）も含まれる。クエリと同じ「　　　　　」を持つため、ペアとして追跡可能",
        "choices": ["Session ID", "Transaction ID", "Sequence Number", "Port Number", "Query Name"],
        "correctIndex": 1
    },
    {
        "text": "あるドメイン名を別のドメイン名に転送する際に使われるレコード名は何か",
        "choices": ["Aレコード", "AAAAレコード", "CNAMEレコード", "MXレコード", "PTRレコード"],
        "correctIndex": 2
    },
    {
        "text": "そのドメインのメールを受け取るサーバーを指定するのに使われるレコード名は何か",
        "choices": ["Aレコード", "AAAAレコード", "CNAMEレコード", "MXレコード", "PTRレコード"],
        "correctIndex": 3
    },
    {
        "text": "IPアドレス→ドメイン名の逆引きに使われるレコード名は何か",
        "choices": ["Aレコード", "AAAAレコード", "CNAMEレコード", "MXレコード", "PTRレコード"],
        "correctIndex": 4
    },
    {
        "text": "DNSでUDPは事前手続きなしに問い合わせを送れるためメリットが多い。誤っているのはどれか",
        "choices": ["名前解決を素早く行うことができる", "サーバー側の負荷を抑えやすい", "多数の問い合わせを効率よく処理しやすい", "通信内容の完全な到達保証が行われる", "軽量な通信が可能である"],
        "correctIndex": 3
    },
    {
        "text": "DNSでTCPが使われるのはどのような時か一番正しいと思うものを答えよ",
        "choices": ["名前解決を素早く行うことができる", "サーバー側の負荷を抑えやすい", "多数の問い合わせを効率よく処理しやすい", "ゾーン転送を行う場合"],
        "correctIndex": 3
    }
];